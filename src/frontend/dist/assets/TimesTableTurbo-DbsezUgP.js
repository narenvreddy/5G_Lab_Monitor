...
function genQ(difficulty) {
  const maxTable = difficulty === 0 ? 4 : difficulty === 1 ? 7 : 12;
  ...
  while (s.size < 4 && attempts < 100) {
    s.add(randInt(2, maxTable) * randInt(1, 12));
    attempts++;
  }
  while (s.size < 4) s.add(answer + s.size);
  const choices = shuffle([...s]);
  return { a, b, answer, choices, correct: choices.indexOf(answer) };
}
function TimesTableTurbo({ difficulty, onGameOver }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col items-center p-4 gap-5 pt-6", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        className: `w-full max-w-sm py-10 rounded-3xl text-center shadow-md transition-colors ${flash === "correct" ? "bg-[#00C9A7]" : flash === "wrong" ? "bg-[#EF476F]" : "bg-white"}`,
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "font-black text-6xl text-[#1A1A2E]", children: [
            q.a,
            " × ",
            q.b
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[#6B6B8A] font-bold mt-2", children: "= ?" })
        ]
      }
    ),

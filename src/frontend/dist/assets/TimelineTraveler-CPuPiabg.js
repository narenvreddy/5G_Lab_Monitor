...
function TimelineTraveler({ difficulty, onGameOver }) {
  ...
  const [slotIds] = reactExports.useState(() => facts.map((f) => `slot-${f.id}`));
  reactExports.useEffect(() => {
    if (gameOver) onGameOver(score);
  }, [gameOver, score, onGameOver]);
  const handleSelectSource = (fact) => {
    if (checked) return;
    if ((selected == null ? void 0 : selected.id) === fact.id) {
      setSelected(null);
    } else {
      setSelected(fact);
    }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col min-h-[520px] bg-gradient-to-b from-[#F4F2FF] to-[#EDF0FF] p-4 gap-4 select-none", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("style", { children: TIMELINE_CSS }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-black text-[#1A1A2E] text-base text-center", children: "Put these math history facts in order — earliest first!" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-2", children: unplaced.map((fact) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        type: "button",
        ...
        className: `p-3 rounded-2xl text-left font-bold text-sm shadow-md transition-all min-h-[44px] ${(selected == null ? void 0 : selected.id) === fact.id ? "bg-[#5B4FCF] text-white scale-[1.02]" : "bg-white text-[#1A1A2E] border-2 border-[#1A1A2E]/10 hover:border-[#5B4FCF] active:scale-95"}`,
        children: fact.text
      },
      fact.id
    )) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-bold text-[#6B6B8A] text-center", children: "← Earliest → Latest" }),
      slotIds.map((slotId, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
          "data-ocid": `timeline.slot.item.${i + 1}`,
          onClick: () => handleSlotTap(i),
          className: `p-3 rounded-2xl text-left text-sm font-bold min-h-[44px] border-2 transition-all ${ordered[i] ? checked ? result[i] ? "bg-[#00C9A7] border-[#00C9A7] text-white" : "bg-[#EF476F] border-[#EF476F] text-white" : "bg-[#FFD166] border-[#FFD166] text-[#1A1A2E] active:scale-95" : "bg-white border-dashed border-[#5B4FCF]/40 text-[#6B6B8A]"}`,
          children: ordered[i] ? `${i + 1}. ${ordered[i].text}${checked ? ` (${ordered[i].label})` : ""}` : `${i + 1}. Tap a card, then tap here`
        },
        slotId
      ))
    ] }),
    ...
    checked && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "font-black text-2xl text-[#5B4FCF]", children: [
      result.filter(Boolean).length,
      "/",
      facts.length,
      " correct! +",
      result.filter(Boolean).length * 20,
      " pts"
    ] }) })
  ] });
}

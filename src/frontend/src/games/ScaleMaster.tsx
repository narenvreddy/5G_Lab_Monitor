...
function BalanceScale({ tilt }: { tilt: "left" | "right" | "balanced" }) {
  ...
  return (
    <svg width="200" height="120" viewBox="0 0 200 120" aria-hidden="true">
      ...
      />
      <circle cx="100" cy="55" r="6" fill="#5B4FCF" />
      <rect
        x="15"
        y={leftY}
        width="50"
...
export function ScaleMaster({ difficulty, onGameOver }: GameProps) {
  const [timeLeft, setTimeLeft] = useState(60);
  const [score, setScore] = useState(0);
  const [done, setDone] = useState(false);
  const [q, setQ] = useState(() => genQ(difficulty));
  const [flash, setFlash] = useState<"correct" | "wrong" | null>(null);
  const [tilt, setTilt] = useState<"left" | "right" | "balanced">("left");
  ...
  return (
    <div className="flex flex-col items-center p-4 gap-5 pt-6">
      ...
      <div
        className={`w-full max-w-sm rounded-3xl py-4 shadow-md text-center transition-colors ${
          flash === "correct"
            ? "bg-[#00C9A7]"
            : flash === "wrong"
              ? "bg-[#EF476F]"
      ...
      >
        <p className="font-black text-4xl text-[#1A1A2E]">{q.text}</p>
        <div className="flex justify-center mt-2">
          <BalanceScale tilt={tilt} />
        </div>
      </div>
